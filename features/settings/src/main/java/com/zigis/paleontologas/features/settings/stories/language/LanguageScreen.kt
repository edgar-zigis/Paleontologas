...
import com.zigis.paleontologas.features.settings.stories.language.list.LanguageListItemView
...
fun LanguageScreen(
    ...
    LifecycleEventHandler {
        if (it == Lifecycle.Event.ON_START) {
            viewModel.intents.sendSafely(LanguageScreenIntent.Initialize)
        }
    }
}
...
private fun LanguageScreenUiImplementation(
    ...
    sendIntent: (LanguageScreenIntent) -> Unit?
) {
    ...
    NavigableScaffold(
        ...
        onBack = {
            sendIntent(LanguageScreenIntent.InvokeBack)
        }
    ) {
        LazyColumn(
            modifier = Modifier
            ...
            items(items) { item ->
                LanguageListItemView(
                    ...
                    onClick = {
                        sendIntent(LanguageScreenIntent.ChangeLocale(locale = item.locale))
                    }
                )
            }
        }
